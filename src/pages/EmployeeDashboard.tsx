 ...
 * Fixed: UI not rendering — was caused by User interface mismatch (missing id field set)
 * and incorrect role redirect path ("/student/dashboard" doesn't exist).
 *
 * DESIGN NOTES (this pass):
 * - Heading split into a small "Welcome back" eyebrow above a plain page title, instead of
 *   one crowded line with a bullet separator.
 * - Profile card actions are icon + label buttons in a 3-up grid.
 * - "Ready for pickup" banner now uses the success color token instead of hardcoded green,
 *   matching the rest of the design system.
 * - Application rows use a left accent bar colored by status (reused for every status, not
 *   just a one-off green background for ready-for-pickup).
 * - Loading state for the applications list is a skeleton instead of plain text.
 */
...
import { BarChart3, Bell, Calendar, FilePlus2, FileText, Inbox, PackageCheck, Search, UserCircle } from "lucide-react";
...
const EmployeeDashboard = () => {
  ...
  useEffect(() => {
    const loadUser = async () => {
      ...
      if (profile.role === "student") { navigate("/dashboard", { replace: true }); return; }
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <Navbar
        isLoggedIn={!!user}
        userRole={(user?.role as "student" | "employee" | "staff") ?? "employee"}
        ...
        <div className="container mx-auto px-4 py-8">
          ...
          <Alert className="mb-6 border-warning bg-warning/10">
            ...
            <AlertDescription className="text-warning-foreground">
              Your contract expires on <strong>July 15, 2026.</strong> Please process renewal soon.
            </AlertDescription>
          </Alert>
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4 mb-8">
            ...
            <DashboardCard title="Contract Info" description="View contract details" icon={Calendar} iconClassName="text-accent" onClick={() => navigate("/contract")} />
